...
export function Sidebar() {
  ...
  const credits = (user as any)?.credits || 100;
    ...
    <aside className="fixed left-0 top-0 h-full w-64 flex flex-col border-r border-white/5 bg-[#080c14] z-50">
      ...
      <div className="flex items-center gap-3 px-6 py-5 border-b border-white/5">
        ...
        <div>
          ...
          <span className="ml-1 text-[10px] font-medium text-indigo-400 bg-indigo-400/10 px-1.5 py-0.5 rounded-full">
            AI
          </span>
        </div>
      </div>
        ...
        {NAV_ITEMS.map(({ href, icon: Icon, label }) => {
          ...
          return (
            ...
            >
              ...
              {isActive && (
                <ChevronRight className="w-3 h-3 ml-auto text-indigo-400/60" />
              )}
            </Link>
          );
        })}
